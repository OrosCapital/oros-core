...
<div class="row">
    ...
    <div class="col-xs-12">
        ...
        <div class="table-responsive">
            <table id="vendor-list" class="table table-striped table-bordered table-hover">
                <thead>
                <tr>
                ...
                <g:each in="${dataReturns}" var="vendors" status="i">
                    <tr>
                        <td>${vendors[0]}</td>
                        <td>${vendors[1]}</td>
                        <td>${vendors[2]}</td>
                        ...
                        <td>${vendors[4]}</td>
                        <td>
                            <sec:access controller="vendor" action="update">
                                <span class="col-xs-6"><a href="" vendorId="${vendors.DT_RowId}" class="edit-vendor" title="Edit"><span class="green glyphicon glyphicon-edit"></span></a></span>
                            </sec:access>
                            <sec:access controller="vendor" action="deleteVendor">
                                <span class="col-xs-6"><a href="" vendorId="${vendors.DT_RowId}" class="delete-vendor" title="Delete"><span class="red glyphicon glyphicon-trash"></span></a></span>
                            </sec:access>
                        </td>
                    </tr>
                </g:each>
...
<script type="text/javascript">
    jQuery(function ($) {
        var oTable1 = $('#vendor-list').dataTable({
...
//                    $('#'+updateDiv).html(data);
        ...
        $('#vendor-list').on('click', 'a.edit-vendor', function (e) {
            var control = this;
        ...
        $('#vendor-list').on('click', 'a.delete-vendor', function (e) {
            var control = this;
            var vendorId = $(control).attr('vendorId');
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    if(data.isError==false){
                        $("#vendor-list").DataTable().row(selectRow).remove().draw();
                    }
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                }
